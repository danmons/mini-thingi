...
## Begin WINE downloads

for RARCH in i386
do

for DISTRO in bullseye
do

  echo "__START__ download of WINE packages for ${RARCH}"
  ...
  # Build sources.list
  > "${SL}"
  echo "deb ${WINEMIRROR} ${DISTRO} main" >>"${SL}"
  ...
  # Prepare WINE keys
  rm -f "${ADIR}/etc/apt/trusted.gpg.d/winehq.gpg"
  curl --silent https://dl.winehq.org/wine-builds/winehq.key | gpg --dearmor -o "${ADIR}/etc/apt/trusted.gpg.d/winehq.gpg"

  ## Grab Debian gpg keys for mirros
  #cd "${ADIR}"
  ...
  apt-get -c="${ACONF}" download -y $( cat "${MDIR}/conf/wine_packages_${DISTRO}_${RARCH}" | tr '\n' ' ' )
